...
public class BasicEdgeOrchestrator extends EdgeOrchestrator {
	...
	public void initialize() {
		...
		numberOfHost = SimSettings.getInstance().getNumOfEdgeHosts(); // #hosts (in edge_devices.xml there are 14
																		// datacanters with 1 host/datacenter
		// System.out.println("---- EO numberOfHost: "+numberOfHost+" --- datacenters:
		// "+SimSettings.getInstance().getNumOfEdgeDatacenters()+" -- edge VMs:
	...
	public int getDeviceToOffload(Task task) {
		...
		// System.out.println("------ GENERIC_EDGE_DEVICE_ID: before "+result);
		if (!simScenario.equals("SINGLE_TIER")) {
			// decide to use cloud or Edge VM
			int CloudVmPicker = SimUtils.getRandomNumber(0, 100);

			if (CloudVmPicker <= SimSettings.getInstance().getTaskLookUpTable()[task.getTaskType()][1]) // probability
																										// of cloud
																										// selection
																										// (based on
																										// type of
	...
	public Vm getVmToOffload(Task task, int deviceId) {
		...
		if (deviceId == SimSettings.CLOUD_DATACENTER_ID) {
			...
			for (int hostIndex = 0; hostIndex < list.size(); hostIndex++) {
				...
				// number of cloud hosts and VMs is defined in default_config.properties
				for (int vmIndex = 0; vmIndex < vmArray.size(); vmIndex++) {
					...
					// System.out.println("--- requiredCapacity: "+requiredCapacity+"
					// targetVmCapacity "+targetVmCapacity);
			...
			selectedVM = selectVmOnHost(task); // Edge devices
	...
	public EdgeVM selectVmOnHost(Task task) {
		...
		if (policy.equalsIgnoreCase("RANDOM_FIT")) {
			...
			double targetVmCapacity = (double) 100
					- vmArray.get(randomIndex).getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
			...
			while (tries < vmArray.size()) {
				...
				double targetVmCapacity = (double) 100 - vmArray.get(lastSelectedVmIndexes[relatedHostId])
						.getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
				if (requiredCapacity <= targetVmCapacity) {
					// System.out.println("Herreeeee Host @"+lastSelectedVmIndexes[relatedHostId]);
		...
		if (policy.equalsIgnoreCase("RANDOM_FIT")) {
			...
			while (selectedVM == null && hostCheckCounter < numberOfHost) {
				...
				while (tries < vmArray.size()) {
					...
					double targetVmCapacity = (double) 100 - vmArray.get(lastSelectedVmIndexes[lastSelectedHostIndex])
							.getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
					if (requiredCapacity <= targetVmCapacity) {
