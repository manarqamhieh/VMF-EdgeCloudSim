...
import edu.boun.edgecloudsim.applications.sample_app1.BestVirtualMachineFit;
		...
		for(int j=SS.getMinNumOfMobileDev(); j<=SS.getMaxNumOfMobileDev(); j+=SS.getMobileDevCounterSize())
		{
			for(int k=0; k<SS.getSimulationScenarios().length; k++)
			{
				for(int i=0; i<SS.getOrchestratorPolicies().length; i++)
				{
					...
					// Here is the end of a simulation iteration.
					
					BestVirtualMachineFit bvmf = new BestVirtualMachineFit();
					bvmf.startAlgorithm();
