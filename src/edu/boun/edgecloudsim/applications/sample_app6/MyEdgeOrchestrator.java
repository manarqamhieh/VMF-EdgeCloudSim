...
public class MyEdgeOrchestrator extends EdgeOrchestrator {
	...
	public void initialize() {
		...
		numberOfHost=SimSettings.getInstance().getNumOfEdgeHosts(); 	// #hosts (in edge_devices.xml there are 14 datacanters with 1 host/datacenter
		//System.out.println("---- EO numberOfHost: "+numberOfHost+" --- datacenters: "+SimSettings.getInstance().getNumOfEdgeDatacenters()+" -- edge VMs: "+SimSettings.getInstance().getNumOfEdgeVMs());
	...
	public int getDeviceToOffload(Task task) {
		// can use also, SimSettings.MOBILE_DATACENTER_ID
		// local execution, if local execution time < W
		// otherwise, offload
		// we need to make sure of simScenario. 
		// result is the constants defined in SimSettings
		int result;
		System.out.println("MyEdgeOrchestrator.getDeviceToOffload() -------- hellloooooo");
		System.out.println(task);
		double taskTime = task.getCloudletLength()/SimSettings.getInstance().getMipsForMobileVM();
		if(taskTime <= SimSettings.getInstance().W) {
			result = SimSettings.MOBILE_DATACENTER_ID;
		}else {
			result = SimSettings.GENERIC_EDGE_DEVICE_ID;
		}
		System.out.println("result = "+result+"  tasktime = "+taskTime+" -- w = "+SimSettings.getInstance().W);
		return result;
	}
...
//		System.out.println("MyEdgeOrchestrator.selectVmOnHost() --------- here");
