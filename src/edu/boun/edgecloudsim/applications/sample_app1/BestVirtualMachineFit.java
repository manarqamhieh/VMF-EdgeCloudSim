...
public class BestVirtualMachineFit {
	...
	public void startAlgorithm() {
		...
		try {
			...
			String[] header = { "W", "A", "B", "#tasks", "#onMobile", "#offloaded", "#Success%", "#Failed%", "utilizedVMs", "totalVMs" , "LB_VMs"};
			...
			double w = 6; // like other apps
			// double maxW = SimSettings.W * 300;
			double maxW = SimSettings.getInstance().getSimulationTime();
...
//		SimSettings.W = sum/100.0;
				...
				for (TaskProperty task : tmpTaskList) {
					...
					Arrays.fill(remVM, w + 1);

					// decide if the the task will be offloaded of not
					if (task.getExecutionTimeonMobile() > w) {
						...
						double minRemVM = w + 1; // highest remaining time
						int minIndex = -1;
...
//					System.out.println("VM assignment failed");
				...
				String filename = "sim_results/ite1/BVMF_W_" + w + "_alpha" + SimSettings.alpha + "_beta"
				...
				for (int i = 0; i < VMList.size(); i++) {
					...
					if (vmLoadRes[5].equals("edgeVM")) {
						edgeVMCount++;
						edgeVMExec += (w - VMList.get(i).getRem());
					} else if (vmLoadRes[5].equals("CloudVM")) {
						cloudVMCount++;
						cloudVMExec += (w - VMList.get(i).getRem());
					}
				...
				vmAvgLoad[0] = Double.toString(w);
				vmAvgLoad[1] = Double.toString((edgeVMExec / edgeVMCount) / w * 100);
				vmAvgLoad[2] = Double.toString((cloudVMExec / cloudVMCount) / w * 100);

				csvwriter3.writeNext(vmAvgLoad);

								results[8] = Integer.toString(utilizedVMs);
				results[9] = Integer.toString(edgeVMCount+cloudVMCount);
				results[10] = Double.toString(SimSettings.LB/w);
				
				csvwriter.writeNext(results);

				String ff = "sim_results/ite1/BVMF_failed_W_" + w + "_alpha" + SimSettings.alpha + "_beta"
						+ SimSettings.beta + "_" + SimSettings.getInstance().taskList2.size() + "_" + count + ".log";
