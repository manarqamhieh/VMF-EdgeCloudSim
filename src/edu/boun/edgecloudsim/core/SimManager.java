...
public class SimManager extends SimEntity {
	...
	public void startSimulation() throws Exception{
		...
		File file = new File("sim_results/ite1/BVMF_createdTasks_log.csv");
		FileWriter outputfile = new FileWriter(file);
		createdTasksWriter = new CSVWriter(outputfile);
		String[] header = { "time", "task index" };
		createdTasksWriter.writeNext(header);
		...
		synchronized(this){
			...
			case CREATE_TASK:
				try {
					...
					if (CloudSim.clock()>= tickTime) {
						String [] data = new String [2];
						data[0] = Double.toString(tickTime);
						data[1] = Integer.toString(taskCount);
						createdTasksWriter.writeNext(data);
						taskCount=0;
						tickTime +=SimSettings.incr;
